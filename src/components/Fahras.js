    ...
    return (
        <>
                        ...
                        <Table striped bordered hover>
                            <thead>
                                <tr className="bg-dark">
                                    ...
                                    <th className="fw-bold text-center d-none d-md-table-cell">إسم السورة بلانجليزية</th>
                                    ...
                                    surahs?.map((surah, index) => {
                                        return (
                                            <tr key={surah.id}>
                                                ...
                                                <td className="text-center d-none d-md-table-cell">{surah.name_simple}</td>
                                                <td className="text-center"><button className="btn custom-button fw-bold px-3" onClick={() => { handleShow(surah.id) }}>قراءة</button> </td>
                                                <td className="text-center">{surah.revelation_place === "Meccan" ? "مكية" : "مدنية"}</td>
            ...
            <Modal show={show} onHide={handleClose}>
                <Modal.Body className="overflow-y-auto overflow-x-hidden p-0" style={{ height: "80vh" }}>
                    <SurahReader id={surahId} />
                </Modal.Body>
                <Modal.Footer className="text-star">
                    <Button variant="danger" className="fw-bold w-50 m-auto rounded-0" onClick={handleClose}>
                        إغلاق
                    </Button>
                </Modal.Footer>
            </Modal>
